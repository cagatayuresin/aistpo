...
def train_and_evaluate(model, name, X_train, X_test, y_train, y_test, le, feature_names):
...
def main():
    X_train, X_test, y_train, y_test, le, feature_names = get_preprocessed_data()
    ...
    models = {
        ...
        # SVM uzun surebilir ama istendi. 
    ...
    for name, model in models.items():
        acc, f1 = train_and_evaluate(model, name, X_train, X_test, y_train, y_test, le, feature_names)
    ...
    with open(os.path.join(comp_dir, "model_comparison_report.md"), "w", encoding='utf-8') as f:
        f.write("# CI/CD failure_type Tahmini - Modellerin Karsilastirilmasi\n\n")
        f.write("Bu belgede 6 farkli makine ogrenmesi modelinin 10 sinifli `failure_type` tahmini uzerindeki performanslari ozetlenmistir.\n\n")
    ...
    print("\nTum egitimler ve raporlamalar basariyla tamamlandi!")
