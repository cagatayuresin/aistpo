...
def train_and_evaluate(model, name, X_train, X_test, y_train, y_test, feature_names):
    ...
    # Hedef 0/1 binary 
    classes = ["Non-GodClass(0)", "GodClass(1)"]
...
def main():
    ...
    with open(os.path.join(comp_dir, "model_comparison_report.md"), "w", encoding='utf-8') as f:
        f.write("# SQA GodClass Tahmini - Modellerin Karsilastirilmasi\n\n")
        f.write("Bu belgede 5 farkli makine ogrenmesi modelinin `GodClass` ihtiyaci uzerindeki (SMOTE eklenmis) performanslari ozetlenmistir.\n\n")
